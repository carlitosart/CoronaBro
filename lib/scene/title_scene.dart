...
class TitleScene extends Scene with KeyBoardWatcher {
  ...
  Future preload() async {
    // load resouce;
    int current = 0;
    int total = 11 + 8; //resoureProvider+audioManager
  ...
  _drawTranslation(CanvasWrapper ctx) {
    double f = fraction ?? 0.0;
    Image.asset('assets/resource/bomb1.png');
    ctx.setBrush("#ccc");
    ctx.setFont("80px Pokemon Bold");
    String msg = "Super Corona Bros";
    num fontWidth = ctx.measureText(msg);
    ctx.rclearRect(-1, -1);
    ctx.save();
    num transX = ctx.width * .5 - fontWidth * .5;
    ...
    ctx.rfillText(0, 0, msg);
    ctx.restore();
    ctx.setFont("90px Pokemon regular");

    String msg2 = "Presione para continuar";
    String msg3 = "Dispositivos Moviles";
    ctx.save();
    ctx.setBrush("rgba(255,0,0,${0.8 * f})");
