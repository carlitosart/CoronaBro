...
class Stage extends Scene {
  ...
  Future _init() async {
    ...
    int wi = _random.nextInt(sm.width);
    int he = _random.nextInt(sm.height);

    for (var i = 0; i < sm.width; ++i) {
      int as1 = _random.nextInt(19);
      int as2 = _random.nextInt(14);
      for (var j = 0; j < sm.height; ++j) {
        var wallPhase = sm.wallPhase[i][j];
        if (wallPhase == 0x7f) {
          //wall1;
          var softWallBlock =
              WeakBrick("block${i}-${j}", context, this, as1, as2);
        ...
        if (monsterPhase > 0) {
          ...
          switch (monsterType) {
            case 0:
              heroList.add(Hero("hero${i}-${j}", context, this, wi, he));
              break;
            case 1:
            case 2:
            case 3:
        ...
        if (treasurePhase > 0) {
          ...
          switch (treasureType) {
            case 0:
              treasureList
                  .add(TreasureGate("tr${i}-${j}", context, this, i, j));
              break;
            case 1:
              treasureList
                  .add(TreasureBombCountUp("tr${i}-${j}", context, this, i, j));
              break;
            case 2:
              treasureList
                  .add(TreasurePowerUp("tr${i}-${j}", context, this, i, j));
              break;
            case 3:
              treasureList
                  .add(TreasureSpeedUp("tr${i}-${j}", context, this, i, j));
              break;
            case 4:
              treasureList.add(
  ...
  Future _drawStatic() async {
    ...
    Canvas rawCanvas = Canvas(
        rec,
        Rect.fromLTWH(
            0.0, 0.0, context.p_width.toDouble(), context.p_height.toDouble()));
    ...
    paint.shader = ImageShader(
        await picture.toImage(
            context.p_width.toInt(), context.p_height.toInt()),
        TileMode.clamp,
        TileMode.clamp,
